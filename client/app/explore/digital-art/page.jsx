...
export default function DigitalArtPage() {
    const [posts] = useState(
        Array.from({ length: 12 }).map((_, i) => ({
            _id: `art-${i}`,
            content: `Amazing digital artwork showcasing creativity and innovation #${i + 1}`,
            user: { name: `Artist ${i + 1}`, avatar: `https://ui-avatars.com/api/?name=Artist+${i + 1}&background=random` },
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                            <h1 className="text-4xl font-bold text-slate-800">Digital Art</h1>
                            <p className="text-lg text-slate-600">Explore stunning digital artworks and creative designs</p>
                        </div>
                    </div>
                </motion.div>
                        ...
                        <motion.div key={post._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}>
                            <GlassCard className="h-full hover:shadow-xl transition-all cursor-pointer group">
                                <div className="relative h-48 mb-4 overflow-hidden rounded-xl">
                                    <img src={post.image} alt="Art" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                                </div>
